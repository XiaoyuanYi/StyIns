...
* split the the unpaired corpus and the paired one (if available) into training and validation sets in json format, then put them into the StyIns/corpus/ directory. Put the testing input file into the StyIns/inps/ directory.. We provide some examples in these two directories.
...
The source encoder, style encoder and decoder will be pre-trained as a denoising seq2seq model, and then the StyIns model is trained based on the pre-trained one.

One can also edit StyIns/sources/**config.py**  to modify the configuration, such as the hidden size, embedding size, number of instances, ratio of paired data to be sued, data path, training epoch, learning rate and so on.
...
You can also use the parameter *-e* to specify the checkpont to be used, e.g., *-e 10* indicates using the checkpoint saved in the 10-th training epoch. The generated files are saved in StyIns/outs/.
...
## 7. Cite
When you use our source code, we will appreciate it if you could kindly cite our paper:
